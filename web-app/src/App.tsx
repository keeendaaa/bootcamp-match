...
const AUTO_DEMO_MODE = (import.meta.env.VITE_AUTO_DEMO as string | undefined) === 'true';
...
export default function App() {
  ...
  const [token, setToken] = useState<string>(() => (AUTO_DEMO_MODE ? DEMO_TOKEN : localStorage.getItem(AUTH_STORAGE_KEY) || ''));
  const [currentUser, setCurrentUser] = useState<ApiUser | null>(null);
  const [authReady, setAuthReady] = useState(false);
  const [showOnboarding, setShowOnboarding] = useState<boolean>(() => !AUTO_DEMO_MODE && !localStorage.getItem(ONBOARDING_SEEN_KEY));
